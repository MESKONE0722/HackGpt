...
setup_postgresql() {
    ...
    case $OS in
        "Debian/Ubuntu"|"Linux")
            ...
            sudo -u postgres psql -c "CREATE USER hackgpt WITH PASSWORD 'change-me-db-password';" 2>/dev/null || warn "User hackgpt already exists"
        ...
        "macOS")
            ...
            psql -d postgres -c "CREATE USER hackgpt WITH PASSWORD 'change-me-db-password';" 2>/dev/null || warn "User hackgpt already exists"
...
[database]
url = postgresql://db_user:db_password@localhost:5432/hackgpt
